# Onboarding: Beehive Agent Platform
...
```bash
beehive run --scheduler inline --vector memory --query "What is 2+2?"
```
...
```bash
beehive init-tenant --org "My Org" --hive "Main Hive"
```

## 6. Control Dashboard

Open `http://localhost:8787/dashboard`. Register or sign in to view channels, templates, and orgs. Configure channel allowlists via CLI (`beehive channels set`).

## 7. Channels (Slack, Discord)

1. Configure Beekeeper API: `beehive up --with-beekeeper`
2. Set channel secrets: `beehive channels set slack '{"slack_bot_token":"xoxb-...","slack_signing_secret":"..."}'`
