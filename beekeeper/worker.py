    ...
    def _build_patterns(cls) -> None:
        ...
        cls._FILE_PATTERNS = [
            ...
            ("append_file", re.compile(
                ...
                re.IGNORECASE | re.DOTALL), 2, 1),
            # "open/read/show/display/view foo.txt"
            ("read_file", re.compile(
                r'(?:open|read|show|display|cat|print|view)\s+(?:(?:the|file)\s+)?["\']?(\S+\.\S+)["\']?\s*$',
                re.IGNORECASE), 1, None),
        ]
...
class BashWorker(BaseSpecialistWorker):
    """Executes shell commands on the local system. Converts natural-language
    requests to bash via LLM, safety-checks, then runs via subprocess."""

    worker_kind = WorkerKind.bash
    output_model = BashWorkerOutput

    _SYSTEM_PROMPT = textwrap.dedent("""\
        You are a bash command generator. Convert the user's request to a single,
        correct bash command. Rules:
        - Output ONLY the bash command — no explanation, no markdown, no backticks
        - Use ~ for the user's home directory
        - Prefer safe operations; never destructively wipe data
        - For multi-step operations, use && or ; to chain commands
        - If unsure, output: echo "Cannot safely convert this request to a command"
    """)

    def __init__(self, llm_router: LLMRouter, default_working_dir: str | None = None) -> None:
        self.llm_router = llm_router
        self.default_working_dir = default_working_dir or os.path.expanduser("~")
    ...
    def execute(self, task: TaskEnvelope, context: WorkerContext) -> dict[str, Any]:
        query = str(task.payload.get("query") or task.payload.get("command") or "").strip()
        working_dir = str(task.payload.get("working_dir") or self.default_working_dir)
        working_dir = os.path.expanduser(working_dir)

        # Step 1: Get bash command — either pre-specified or generated by LLM
        prebuilt_cmd = str(task.payload.get("command") or "").strip()
        if prebuilt_cmd and not task.payload.get("query"):
            command = prebuilt_cmd
            source = "payload"
        else:
            self._emit_status(context, "Generating shell command…")
            command, source = self.llm_router.call(
                prompt=query,
            ...
            if not command:
                return {
                    "success": False, "error": "LLM did not generate a command",
                    "command": "", "stdout": "", "stderr": "", "returncode": -1,
                    "assistant_reply": "I could not determine a shell command for that request.",
                }
        ...
        if not safe:
            return {
                "success": False, "error": reason, "command": command,
                "stdout": "", "stderr": "", "returncode": -1,
                "assistant_reply": f"I blocked that command for safety: {reason}",
            }

        # Step 3: Execute
        self._emit_status(context, f"Running: {command[:80]}…")
        try:
            ...
            return {
                "success": False, "error": "Command timed out (30s)",
                "command": command, "stdout": "", "stderr": "", "returncode": -1,
                "assistant_reply": f"The command `{command}` timed out after 30 seconds.",
            }
        except Exception as exc:
            return {
                "success": False, "error": str(exc), "command": command,
                "stdout": "", "stderr": "", "returncode": -1,
                "assistant_reply": f"Failed to run command: {exc}",
            }
...
class WorkerRuntime:
    ...
    def __init__(
        ...
        # BashWorker shares the ForgedWorker's LLM router for command generation
        self._workers[WorkerKind.bash] = BashWorker(
            llm_router=self._workers[WorkerKind.forged].llm_router,  # type: ignore[attr-defined]
        )
        if extra_workers:
